...
- **GLM / DeepSeek 修复**：自动为 GLM、Z.AI、DeepSeek 关闭思考模式 — 修复 `glm-4.7-flash` 等模型的慢响应与「No content in streaming response」报错。
- **Gemini**：使用 `thinking_level: minimal`（最低档）以加快提交信息生成。
...
2. **DeepSeek V4 Flash**（`deepseek-v4-flash`）— 全球付费档综合最优。扩展已自动关闭思考模式，降低延迟与成本。
