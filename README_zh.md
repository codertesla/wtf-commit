...
- **暂存提交更安全**：当暂存区已有内容时，生成结果只聚焦暂存变更，不再混入未跟踪文件。
- **Diff 上下文更稳**：未跟踪文件现在会先检测疑似二进制内容，并过滤更多生成产物。
- **运行时兼容性清理**：API 请求不再依赖全局 `fetch`，Node 类型也已对齐插件支持的 VS Code 宿主范围。
- **插件图标更轻量**：发布用 PNG 图标已调整为 256x256，体积更小且保持清晰。
