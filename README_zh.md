...
- **工作区确认**：暂存区为空且关闭 Auto Commit 时，基于工作区生成前会先确认。
- **轻量暂存快照**：Auto Commit 用 index 签名 + 内容哈希做提交前安全校验。
- **更干净的流式预览**：SCM 实时预览会去掉完整 markdown fence，与最终规范化一致。
- **结构更清晰**：生成流程拆分为独立模块，并为 diff 来源规划补充单测。
...
- **智能差异化**：优先处理暂存的更改，混合暂存/未暂存状态会先确认；在关闭 Auto Commit 且暂存区为空时，使用工作区变更前会再确认；生成前过滤低价值 diff 噪声。
...
- **轻量纠偏**：优先尝试本地约定式提交修复；仍不规范时支持点击 `AI Repair` 做一次快速修正。
