...
- **设置界面重构**: 大幅精简了插件配置页面，将 16 个散落的 AI 提供商配置项 (Base URL / Model) 合并至单一的 `wtfCommit.providerOverrides` JSON 配置块中。

- **交互流增强 (Edit in Input Box)**: 在开启了自动提交的确认弹窗中新增直接编辑选项，允许你无缝地回到源码管理视图对 AI 生成的内容进行微调再手动提交。
- **智能架构与 Diff 按需缩放**: 全新底层重构，智能跳过无意义的 lock 文件，防止超大型 Commit 挤占 Token。并为深度思考模型 (如 DeepSeek, MiniMax) 自动配置翻倍的最长等待时间。
