...
- **MiMo 服务商**：新增小米 MiMo（`mimo-v2.5`），OpenAI 兼容 API。
- **GLM 默认模型**：切换为 [智谱](https://bigmodel.cn/pricing) 免费的 `glm-4.7-flash`。
- **移除 Moonshot/Kimi**：内置 Moonshot 已下线 — Kimi 写 commit 性价比远低于 Flash 档。仍要用 Kimi 可通过 **Custom** 配置 OpenAI 兼容端点。
- **价格对比表**：README 新增模型成本对比，方便选型。
...
| **GLM** | `glm-4.7-flash` | 免费 | 免费 | ~¥0 | **GLM 默认** — [智谱免费档](https://bigmodel.cn/pricing)（~20 t/s） |
| **OpenAI** | `gpt-5-nano` | $0.05 | $0.40 | ~$0.0003 | 付费输入单价最低；适合小 diff |
| **GLM** | `glm-4.7-flashx` | ¥0.5 | ¥3 | ~¥0.003 | 更快智谱档（~30–40 t/s） |
| **DeepSeek** | `deepseek-v4-flash` | $0.14 | $0.28 | ~$0.0007 | **全球首选** — 快、便宜、质量好 |
...
† 按 **约 5K 输入 + 150 输出 tokens** 估算。`glm-4.7-flashx` 单次约 **¥0.003**。美元行按未命中缓存计价。

MiMo 国内按量（`mimo-v2.5`）：输入 ¥1.00 / 百万 tokens，输出 ¥2.00 / 百万 tokens（[官方定价](https://mimo.mi.com/docs/zh-CN/price/pay-as-you-go)）。

**推荐顺序（速度 + 性价比）：**

1. **GLM-4.7-Flash**（`glm-4.7-flash`）— 智谱**免费**；有 BigModel Key 即可作为默认。需要更快速度可换 **`glm-4.7-flashx`**（¥0.5 / ¥3 每百万 tokens）。
...
官方定价：[DeepSeek](https://api-docs.deepseek.com/quick_start/pricing) · [MiMo](https://mimo.mi.com/docs/zh-CN/price/pay-as-you-go) · [Gemini](https://ai.google.dev/gemini-api/docs/pricing) · [OpenAI](https://developers.openai.com/api/docs/pricing) · [智谱 GLM](https://bigmodel.cn/pricing) · [OpenRouter](https://openrouter.ai/models)
