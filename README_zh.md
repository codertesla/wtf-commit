...
- **生成单飞锁**：生成进行中再次触发会被忽略，避免并行请求与输入框竞态。
- **本地约定式提交修复**：缺空格、全角冒号、超长标题会先本地修正，再考虑 AI Repair。
- **取消/空结果恢复**：取消生成会清掉半截流式预览，并还原你在 SCM 输入框中的原始意图。
- **大 diff 硬截断**：摘要仍受 `maxDiffChars` 限制；AI Repair 也会带上已截断的 diff 上下文。
