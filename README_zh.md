...
- **插件 LOGO 更新**：现在使用更简洁的抽象 commit + sparkle 标识。
- **默认模型已更新**：DeepSeek 默认使用 `deepseek-v4-flash`，MiniMax 默认使用 `MiniMax-M2.7`，Kimi 默认使用 `kimi-k2.6`，GLM 默认使用 `glm-5.1`。
- **默认模型文档同步**：内置服务商表格已同步到当前运行时默认值。
