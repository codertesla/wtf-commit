...
[![GLM 4.7 Flash](https://img.shields.io/badge/GLM-4.7%20Flash%20免费-345FF2?logo=zhipu&logoColor=white)](https://bigmodel.cn/pricing)
[![Z.AI GLM 4.7 Flash](https://img.shields.io/badge/Z.AI-GLM%204.7%20Flash%20免费-1A1A1A)](https://docs.z.ai/guides/overview/pricing)
...
- **API Key 链接**：README 列出各内置服务商的官方 API Key 获取页面（OpenAI、DeepSeek、MiMo、GLM、Z.AI、Gemini、OpenRouter）。
- **市场徽章**：README 徽章突出推荐用于提交信息的模型 — 免费 GLM / Z.AI、DeepSeek V4 Flash、MiMo V2.5、Gemini 3.1 Flash Lite。
...
| **GLM**（国内） | [open.bigmodel.cn/usercenter/apikeys](https://open.bigmodel.cn/usercenter/apikeys) |
...
| 服务商 | 模型 | 输入 | 输出 | 约单次成本† | 适合场景 |
|--------|------|-----:|-----:|------------:|----------|
| **OpenRouter** | `openrouter/free` | $0 | $0 | ~$0 | 零成本试用；质量与延迟不稳定 |
| **GLM** | `glm-4.7-flash` | 免费 | 免费 | ~¥0 | **国内** — [智谱 BigModel 免费档](https://bigmodel.cn/pricing) |
| **Z.AI** | `glm-4.7-flash` | 免费 | 免费 | ~$0 | **国际** — [Z.AI 免费档](https://docs.z.ai/guides/overview/pricing) |
| **OpenAI** | `gpt-5-nano` | $0.05 | $0.40 | ~$0.0003 | 付费输入单价最低；适合小 diff |
| **Z.AI** | `glm-4.7-flashx` | $0.07 | $0.40 | ~$0.0004 | 更快 Z.AI 档 |
| **GLM** | `glm-4.7-flashx` | ¥0.5 | ¥3 | ~¥0.003 | 更快智谱档（~30–40 t/s） |
| **DeepSeek** | `deepseek-v4-flash` | $0.14 | $0.28 | ~$0.0007 | **付费首选** — 快、便宜、质量好 |
| **MiMo** | `mimo-v2.5` | $0.14 | $0.28 | ~$0.0007 | 与 DeepSeek 同档；OpenAI 兼容 |
| **Gemini** | `gemini-3.1-flash-lite` | $0.25 | $1.50 | ~$0.0015 | [免费额度](https://ai.google.dev/gemini-api/docs/pricing) 慷慨 |

† 按 **约 5K 输入 + 150 输出 tokens** 估算。`glm-4.7-flashx` 国内单次约 **¥0.003**。美元行按未命中缓存计价。

> **GLM 与 Z.AI**：同一模型家族、不同平台 — **GLM** 走国内端点（`open.bigmodel.cn`），**Z.AI** 走国际端点（`api.z.ai`）。API Key 不通用。

MiMo 国内按量（`mimo-v2.5`）：输入 ¥1.00 / 百万 tokens，输出 ¥2.00 / 百万 tokens（[官方定价](https://mimo.mi.com/docs/zh-CN/price/pay-as-you-go)）。

**推荐顺序（速度 + 性价比）：**

1. **GLM-4.7-Flash**（`glm-4.7-flash`）— **免费**。国内用 **GLM**（[智谱](https://bigmodel.cn/pricing) Key），国际用 **Z.AI**（[Z.AI](https://docs.z.ai/guides/overview/pricing) Key）。需要更快速度可换同平台的 **`glm-4.7-flashx`**。
2. **DeepSeek V4 Flash**（`deepseek-v4-flash`）— 全球付费档综合最优。建议关闭思考模式（`"thinking": {"type": "disabled"}`）。
3. **MiMo V2.5**（`mimo-v2.5`）— 与 DeepSeek 同美元价位。
4. **Gemini 3.1 Flash Lite**（`gemini-3.1-flash-lite`）— 免费额度适合轻度使用。
5. **`openrouter/free`** — 适合尝鲜。
