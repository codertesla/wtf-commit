...
- **修复 Auto Push 误报失败**: 现在会区分真正的 `git push` 失败，和 VS Code 在推送成功后刷新仓库状态时抛出的错误，不再把后者误提示成推送失败。
- **更安全的恢复策略**: 如果推送大概率已经到达远端，插件不再继续给出 `Undo Commit`，避免用户把本地回退到与远端分叉的状态。
- **更清晰的 Git 错误信息**: 通知会尽量展示底层 Git 命令和 stderr 输出，不再只剩下模糊的 `Failed to execute git`。
