...
- **更清晰的成功态符号**: 生成完成、提交成功、推送进行中、推送成功等状态现在会通过更直观的状态栏图标反馈出来，扫一眼就能看懂。
- **更易读的提交确认**: Auto Commit 确认框改成标题加详情区展示生成结果，长 commit message 的阅读体验更顺手。
- **更高效的多仓库选择**: 多仓库场景下会优先展示目录名，再补充完整路径，定位目标仓库更快。
