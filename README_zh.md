...
- **流式输出**：LLM 响应现在通过 SSE 实时流式返回，生成过程中即可看到进度，体感延迟大幅降低。
- **Temperature 可配置**：新增 `wtfCommit.temperature` 设置项（默认 `0.7`），可自由控制生成创造性。
- **单元测试覆盖**：新增 36 个单元测试，覆盖消息规范化、约定式提交校验、URL 构建、diff 路径过滤等核心逻辑。
- **更严格的 TypeScript 检查**：启用 `noImplicitReturns`、`noFallthroughCasesInSwitch`、`noUnusedParameters`。
...
- **流式生成**：实时流式输出，生成过程中即可看到 commit message 逐步呈现。
